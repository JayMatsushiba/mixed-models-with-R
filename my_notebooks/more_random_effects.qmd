...
AIC(model_ints_only) # 256.0606
AIC(model_with_slopes) # 240.7851, lower is better

```

Okay, I totally overthought this exercise. I simply needed to run the AIC comparisons across the two different models, with the model_with_slopes coming out ahead with the lower AIC value. Why does it have a lower value, meaning it has a better model fit with the data? This is because we generated the original data with random slopes. Therefore, a model that includes random slopes (slopes different by group) would fit the generated data better. It is a bit circular, since we are the ones that generated the data in the first place, but this demonstrates that we can fit models better this way. 